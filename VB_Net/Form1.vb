...
#Region "Variables"
    ...
    Dim ListaEnviar As New AutoCompleteStringCollection()
    ...
    Public Sub Enviar()
        Try
            If comPort.IsOpen Then
                ...
                If Not Txt_enviar.AutoCompleteCustomSource.Contains(Txt_enviar.Text.Trim) Then
                    'Txt_enviar.AutoCompleteCustomSource.Add(Txt_enviar.Text.Trim)
                    ListaEnviar.Add(Txt_enviar.Text.Trim)
                End If
            End If
        Catch ex As Exception
            IO.File.AppendAllText(Application.StartupPath & "\ACP_LOG.txt", String.Format("{0}[{1}]{2}", Environment.NewLine, DateTime.Now.ToString("dd-MM-yyyy HH:mm:ss"), ex.ToString()))
    ...
    Public Sub Actualizar()
        Try
            ...
            If datos_puerto.StartsWith(txt_inicio.Text) Then
                ...
                datos_puerto = datos_puerto.Replace(txt_inicio.Text, "")            'Quito repeticiones para evitar tramas corruptas
                datos_split = datos_puerto.Split(txt_separador.Text)    'Separo los datos
                'UNA MEJORA SERÍA CREAR LAS SERIES EN FUNCIÓN DE LO QUE LEE PARA DETECTAR AUTOMÁTICAMENTE
...
#Region "Código"
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            With My.Application.Info.Version
                Me.Text = "Arduino COM Plotter " & .Major.ToString & "." & .Minor.ToString & "." & .Build.ToString & "      |      Proyecto[sin guardar]"
            End With
            ...
            tbl.Columns.Add("Decimales", GetType(String))
            tbl.Rows.Add(True, "Nombre1", "", "-10", "110", True, True, 1)
            tbl.Rows.Add(True, "Nombre2", "", "-10", "110", True, True, 1)
            tbl.Rows.Add(True, "Nombre3", "", "-10", "110", False, True, 0)
            '
            ...
            For i = 0 To 1
                ...
                Chart1.ChartAreas(i).AxisY.Maximum = 110
                Chart1.ChartAreas(i).AxisY.Minimum = -10
                Chart1.ChartAreas(i).AxisY2.Maximum = 110
                Chart1.ChartAreas(i).AxisY2.Minimum = -10
            ...
            'Autocompletar comandos enviar
            With Txt_enviar
                .AutoCompleteCustomSource = ListaEnviar
            ...
            Next
            '
            Cbo_SincEjes.Items.Add("Ejes Y sincronizados")
            Cbo_SincEjes.Items.Add("Ejes Y independientes")
            Cbo_SincEjes.SelectedIndex = 0
            '
        Catch ex As Exception
            IO.File.AppendAllText(Application.StartupPath & "\ACP_LOG.txt", String.Format("{0}[{1}]{2}", Environment.NewLine, DateTime.Now.ToString("dd-MM-yyyy HH:mm:ss"), ex.ToString()))
    ...
    Private Sub Form1_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        ...
        If result = DialogResult.Yes Then
            '
            If My.Settings.AutoCompleteList.Count > 0 Then My.Settings.AutoCompleteList.Clear()
            For Each item As String In ListaEnviar
                My.Settings.AutoCompleteList.Add(item)
            Next
            My.Settings.Save()
            '
            Me.Dispose()
            ...
            If datosOK = True Then
                ...
                For i = 0 To series.Count - 1
                    ...
                    If (contador >= num_ventana.Value) Then 'Scroll cada x tiempo
                        ...
                        '
                        Chart1.ChartAreas(1).AxisX.Maximum += num_ventana.Value
                        Chart1.ChartAreas(1).AxisX.ScaleView.Zoom(Chart1.ChartAreas(1).AxisX.Maximum - num_ventana.Value, Chart1.ChartAreas(1).AxisX.Maximum)
                        contador = 0
                        Chart1.Invalidate()
                        Timer1.Enabled = True
                    End If
                    'Nombres
                    'https://learn.microsoft.com/es-es/office/vba/language/reference/user-interface-help/vartype-function
                    ...
                    'LOS TRATA SIEMPRE COMO DOUBLE
                    Select Case CInt(dgv_vars.Rows(i).Cells(7).Value)
                        Case 0 'int
                            dgv_vars.Rows(i).Cells(2).Value = CInt(series(i))
                        Case 1
                            dgv_vars.Rows(i).Cells(2).Value = series(i).ToString("###0.0")
                        Case 2
                            dgv_vars.Rows(i).Cells(2).Value = series(i).ToString("####0.00")
                        Case 3
                            dgv_vars.Rows(i).Cells(2).Value = series(i).ToString("#####0.000")
                        Case Else
                            dgv_vars.Rows(i).Cells(2).Value = CInt(series(i))
                    End Select
                    'dgv_vars.Rows(i).Cells(2).Value = series(i).ToString("####0.00")
    ...
    Private Sub dgv_vars_CellBeginEdit(sender As Object, e As DataGridViewCellCancelEventArgs) Handles dgv_vars.CellBeginEdit
        Try
            ...
            If e.ColumnIndex <> 0 AndAlso e.ColumnIndex <> 1 AndAlso e.ColumnIndex <> 5 AndAlso e.ColumnIndex <> 6 AndAlso e.ColumnIndex <> 7 Then
                e.Cancel = True
            End If
        Catch ex As Exception
            IO.File.AppendAllText(Application.StartupPath & "\ACP_LOG.txt", String.Format("{0}[{1}]{2}", Environment.NewLine, DateTime.Now.ToString("dd-MM-yyyy HH:mm:ss"), ex.ToString()))
    ...
    Private Sub Chart1_AxisScrollBarClicked(sender As Object, e As ScrollBarEventArgs)
        Try
            If (e.ButtonType = ScrollBarButtonType.ZoomReset) Then
                e.IsHandled = True
                'If Cbo_SincEjes.SelectedIndex = 0 Then
                Chart1.ChartAreas(0).AxisY.ScaleView.ZoomReset(0)
                Chart1.ChartAreas(0).CursorY.SelectionEnd = Double.NaN
                Chart1.ChartAreas(0).AxisX.ScaleView.Zoom(Chart1.ChartAreas(0).AxisX.Maximum - num_ventana.Value, Chart1.ChartAreas(0).AxisX.Maximum)
                Chart1.ChartAreas(1).AxisY.ScaleView.ZoomReset(0)
                Chart1.ChartAreas(1).CursorY.SelectionEnd = Double.NaN
                Chart1.ChartAreas(1).AxisX.ScaleView.Zoom(Chart1.ChartAreas(1).AxisX.Maximum - num_ventana.Value, Chart1.ChartAreas(1).AxisX.Maximum)
                'ElseIf Cbo_SincEjes.SelectedIndex = 1 Then
            ...
            If h.ChartElementType = DataVisualization.Charting.ChartElementType.AxisLabels AndAlso e.Button = MouseButtons.Left Then
                ...
                If h.Axis.AxisName.ToString = "Y" Then
                    ...
                    If IsNumeric(min) Then
                        ...
                        Else
                            If h.ChartArea.Name = "ChartArea1" Then Chart1.ChartAreas(0).AxisY.Minimum = min
                            If h.ChartArea.Name = "ChartArea2" Then Chart1.ChartAreas(1).AxisY.Minimum = min
                        End If
                    ...
                    If IsNumeric(max) Then
                        ...
                        Else
                            If h.ChartArea.Name = "ChartArea1" Then Chart1.ChartAreas(0).AxisY.Maximum = max
                            If h.ChartArea.Name = "ChartArea2" Then Chart1.ChartAreas(1).AxisY.Maximum = max
                        End If
                        'Chart1.ChartAreas(0).AxisY.Maximum = max
                        'Chart1.ChartAreas(1).AxisY.Maximum = max
                ...
                If h.Axis.AxisName.ToString = "Y2" Then
                    ...
                    If IsNumeric(min) Then
                        ...
                        Else
                            If h.ChartArea.Name = "ChartArea1" Then Chart1.ChartAreas(0).AxisY2.Minimum = min
                            If h.ChartArea.Name = "ChartArea2" Then Chart1.ChartAreas(1).AxisY2.Minimum = min
                        End If
                        'Chart1.ChartAreas(0).AxisY2.Minimum = min
                        'Chart1.ChartAreas(1).AxisY2.Minimum = min
                    End If
                    ...
                    If IsNumeric(max) Then
                        ...
                        If Cbo_SincEjes.SelectedIndex = 0 Then
                            ...
                            Chart1.ChartAreas(1).AxisY2.Maximum = max
                        Else
                            If h.ChartArea.Name = "ChartArea1" Then Chart1.ChartAreas(0).AxisY2.Maximum = max
                            If h.ChartArea.Name = "ChartArea2" Then Chart1.ChartAreas(1).AxisY2.Maximum = max
                        End If
                    End If
                End If
            End If
    ...
    Private Sub Num_vars_ValueChanged_1(sender As Object, e As EventArgs) Handles Num_vars.ValueChanged
        Try
            If dgv_vars.RowCount < Num_vars.Value Then
                tbl.Rows.Add(True, "Nombre" & Num_vars.Value, "", "0", "100", False, True, 1)
                series.Add(0.0)
                '
    ...
    Private Sub Txt_Td_MouseWheel(sender As Object, e As MouseEventArgs) Handles Txt_Td.MouseWheel
        ...
        Simular()
    End Sub
    Private Sub Txt_enviar_KeyDown(sender As Object, e As KeyEventArgs) Handles Txt_enviar.KeyDown
